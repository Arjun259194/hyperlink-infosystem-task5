...
import {
  ...
  GetPostPagination,
  GetTrendingPostsWithPagination,
  LikePost,
  NewPost,
  NewRepostByPostId,
} from "../model/index.js"
...
export default class PostController {
  ...
  static async create(req, res) {
    ...
    if (!userId)
      throw new ErrorResponse("User ID missing from token", 401)

    if (!req.body)
      throw new ErrorResponse("No body provided in request header ")
    ...
    const encres = EncRes("Post created", 201, {
      post: postres,
    })
  ...
  static async update(req, res) {
    ...
    const postupdate = new PostUpdateJson({
      ...req.body,
      user_id: userId,
    })
    ...
    const encres = EncRes("Post Updated", 200, {
      post: updatedpost,
    })
  ...
  static async getPost(req, res) {
    ...
    if (!id || id === "" || id.length <= 0)
      new ErrorResponse(
        "bad request, user id not found or valid",
        400
      )
    ...
    const encres = EncRes("Post found", 200, {
      post,
    })
  ...
  static async getAllPost(req, res) {
    ...
    const encres = EncRes("Posts found", 200, {
      ...pagination.data,
      posts,
    })
  ...
  static async repost(req, res) {
    ...
    const repostjson = new PostRepostJson({
      ...req.body,
      user_id,
    })
    ...
    const encres = EncRes("Repost created", 201, {
      repost,
    })
  ...
  /** @type {ExpressFn} */
  static async getTrendingPostsWithPagination(req, res) {
    const pagination = new PaginationQuery(req.query)
    const posts = await GetTrendingPostsWithPagination(pagination)
    if (posts.length <= 0)
      throw new ErrorResponse("No posts found", 404)

    const encres = EncRes("Found trending posts", 200, {
      posts,
    })
